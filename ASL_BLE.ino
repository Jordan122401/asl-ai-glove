...
//   stream:on         -> start continuous data streaming (for real-time AI)
...
static void bleNotifyLine(const String& rawLine) {
  ...
  for (int i = 0; i < n; i += CHUNK) {
    ...
    pTxChar->setValue((const uint8_t*)part.c_str(), part.length());
    pTxChar->notify();
...
// Read median resistance value from a flex sensor over n samples
float medianFingerRk(int pin,int n){
  const int MAXS=256;                     // Maximum buffer size
  static float buf[MAXS];                 // Static buffer for samples
  if(n>MAXS) n=MAXS;                      // Clamp sample count to buffer size
  
  // Collect n samples
  for(int i=0;i<n;i++){
    uint16_t a=analogRead(pin);           // Read ADC value
    buf[i]=estimateSensorOhms(adcToV(a))/1000.0f; // Convert to kΩ
    delay(1000UL/SAMPLE_HZ);              // Wait for next sample period
  }
  
  // Sort samples using insertion sort
  for(int i=1;i<n;i++){
    float k=buf[i];                       // Current value to insert
    int j=i-1;
    while(j>=0 && buf[j]>k){              // Shift larger values right
      buf[j+1]=buf[j];
      j--;
    }
    buf[j+1]=k;                           // Insert value in sorted position
  }
  
  // Return median value
  return (n&1)?buf[n/2]:0.5f*(buf[n/2-1]+buf[n/2]); // Odd n: middle, even n: average of two middle
}

// Calibrate all five flex sensors
void calibrateFlex(){
  Serial.println("# CAL FLEX: Rest — keep hand relaxed for ~3 s");
  delay(400);                             // Brief pause before starting
  
  const int N=90;                         // Number of samples per calibration (90 samples @ 30Hz = 3 seconds)
  
  // Step 1: Calibrate rest position (all fingers relaxed)
  for(int f=0;f<5;f++)
    R_rest[f]=medianFingerRk(FLEX_PINS[f],N); // Get median resistance at rest
  
  // Step 2: Calibrate maximum bend for each finger individually
  for(int f=0;f<5;f++){
    Serial.printf("# CAL FLEX: Bend FINGER %d to comfortable max (~3 s)\n",f+1);
    delay(400);                           // Pause for user to position finger
    R_maxv[f]=medianFingerRk(FLEX_PINS[f],N); // Get median resistance at max bend
    
    // Warn if range is too small (may indicate bad connection or sensor)
    if (fabs(R_maxv[f]-R_rest[f])<0.8f)
      Serial.printf("# WARN: finger %d range small (ΔR≈%.2f kΩ)\n",f+1,R_maxv[f]-R_rest[f]);
  }
  
  flex_cal_ok=true;                       // Mark calibration as valid
  
  // Build JSON output with calibration values
  String j="{\"R_rest\":[";
  for(int i=0;i<5;i++){
    if(i) j+=", ";
    j+=String(R_rest[i],2);               // Rest resistance with 2 decimal places
  }
  j+="],\"R_max\":[";
  for(int i=0;i<5;i++){
    if(i) j+=", ";
    j+=String(R_maxv[i],2);               // Max resistance with 2 decimal places
  }
  j+="],\"gamma\":"+String(gammaCurve,3)+"}"; // Include gamma value
  
  // Send calibration results to Serial and BLE
  Serial.println(j);
  bleNotifyLine(j);
  Serial.println("# CAL FLEX: done.");
}
...
void calibrateIMU(){
  ...
  if(!adxl_ok){
    Serial.println("# CAL IMU: ADXL345 not detected.");
    bleNotifyLine("# CAL IMU: ADXL345 not detected.");
    return;
  }
  
  Serial.println("# CAL IMU: Place glove FLAT & STILL (~4 s)...");
  bleNotifyLine("# CAL IMU: Place glove FLAT & STILL (~4 s)...");
  delay(300);                             // Brief pause
  
  const int N=120;                        // 120 samples @ 30Hz = 4 seconds
  
  // Discard first few readings (sensor settling time)
  for(int k=0;k<5;k++){
    sensors_event_t e;
    accel.getEvent(&e);
  ...
  // Collect statistics: sum and sum-of-squares for each axis
  double sx=0,sy=0,sz=0,s2x=0,s2y=0,s2z=0;
  for(int i=0;i<N;i++){
    sensors_event_t e;
    accel.getEvent(&e);                   // Read accelerometer
    double x=e.acceleration.x, y=e.acceleration.y, z=e.acceleration.z;
    sx+=x; sy+=y; sz+=z;                  // Accumulate sums
    s2x+=x*x; s2y+=y*y; s2z+=z*z;         // Accumulate squared sums (for std dev if needed)
    delay(1000UL/SAMPLE_HZ);              // Wait for next sample
    saveCalibrationForUser(current_user_name); // Save after each sample (FIXME: should be outside loop)
  }
  ...
  // Store offsets (z-axis should read +g when flat, so offset = measured - g)
  ax_off=mx;                              // X-axis bias
  ay_off=my;                              // Y-axis bias
  az_off=mz-g;                            // Z-axis bias (subtract gravity)
  imu_cal_ok=true;                        // Mark calibration as valid
  
  // Format and send calibration results
  char b[160];
  snprintf(b,sizeof(b),"# CAL IMU OK: ax_off=%.4f ay_off=%.4f az_off=%.4f (m/s^2)",ax_off,ay_off,az_off);
  Serial.println(b);
...
void printHeader_stream(){
  const char* h="t_s,flex1,flex2,flex3,flex4,flex5,roll_deg,pitch_deg,ax_g,ay_g,az_g";
  Serial.println(h);
  bleNotifyLine(String(h));
}
...
bool processCommand(String s){
  ...
  // "stream:on" command: begin continuous streaming mode
  if (s=="stream:on"){
    // Check prerequisites
    if (!flex_cal_ok){
      String m="# ERROR: Run 'cal' first.";
    ...
    if (recording){
      String m="# Busy (trial running). Send 'stop' first.";
      Serial.println(m);
      bleNotifyLine(m);
      return true;
    }
    if (!streaming){
      ...
      next_ms=millis() + period_ms;                   // Initialize timing
      stream_t0_ms=next_ms;               // Set start time reference for streaming
      Serial.println("# STREAM ON");
      bleNotifyLine("# STREAM ON");
      printHeader_stream();               // Print CSV header (no label/trial_id columns)
      delay(2);
    }
    return true;
  }
  ...
  if (s.startsWith("setuser ")){
    ...
    if (loadCalibrationForUser(current_user_name)){
      String m="# Switched to user '"+current_user_name+"' and loaded calibration.";
      Serial.println(m);
      bleNotifyLine(m);
    } else {
      String m="# Switched to user '"+current_user_name+"'. No saved cal yet (run cal / imu_cal).";
      Serial.println(m);
      bleNotifyLine(m);
    }
    return true;
...
void setup(){
  ...
  if (adxl_ok){
    ...
    Serial.println("# WARN: ADXL345 not detected.");
  }
  ...
  Serial.println("# BLE ready. Use Bluetooth LE Explorer.");
  Serial.println("# Commands: start/stop (trial), stream:on/off, rate:<Hz>, cal, imu_cal, label:..., setuser ...");
}
...
void loop(){
  ...
  if (streaming){
    // Streaming mode: output only sensor data (no label/trial_id)
    String s;
    s.reserve(150);                       // Pre-allocate memory
    
    // Build CSV line: time, 5 flex values, roll, pitch, 3 accelerations
    s  = String(t_s,3);  // keep t_s
  s += ","+String(bend_cal[0],3)
     + ","+String(bend_cal[1],3)
     + ","+String(bend_cal[2],3)
     + ","+String(bend_cal[3],3)
     + ","+String(bend_cal[4],3);
