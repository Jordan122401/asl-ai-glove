...
void calibrateIMU(){
  ...
  if (saveCalibrationForUser(current_user_name)) {
    bleNotifyLine("# IMU calibration saved.");
  } else {
    bleNotifyLine("# ERROR: failed to save IMU calibration.");
  }
}
...
if (s=="stream"){
  ...
  if (!streaming){
    ...
    Serial.println("# STREAM ON (windowed, 75 samples per batch, 1s delay between batches)");
    bleNotifyLine("# STREAM ON (windowed, 75 samples per batch, 1s delay between batches)");


    // Announce first batch and send header
    String hdr = "# BATCH " + String(batch_id) + " BEGIN,nsamp=" + String(STREAM_WIN) + ",rate=" + String(SAMPLE_HZ);
    bleNotifyLine(hdr);
    printHeader_stream();  // t_s,flex1..5,roll,pitch,ax,ay,az
    delay(2);
  }
  return true;
}
